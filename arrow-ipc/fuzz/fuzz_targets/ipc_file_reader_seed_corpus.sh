#!/bin/bash -eu
# Build the seed corpus zip for `ipc_file_reader` from arrow-testing.
#
# Usage: ipc_file_reader_seed_corpus.sh <arrow-testing-checkout> <output-zip>

ARROW_TESTING="${1:?arrow-testing checkout path required}"
OUT_ZIP="${2:?output zip path required}"

SRC_DIR="$ARROW_TESTING/data/arrow-ipc-file"
if [ ! -d "$SRC_DIR" ]; then
...
TMP=$(mktemp -d)
trap 'rm -rf "$TMP"' EXIT
# Pull in clusterfuzz testcases and any binary inputs, but skip text metadata.
find "$SRC_DIR" -type f \
...
echo "Built ipc_file_reader seed corpus: $(unzip -l "$OUT_ZIP" | tail -1)"
